...
public class PeerProtocol {
    private static final String TAG = "PeerProtocol";
    ...
    @NonNull
    private static ByteBuffer getTypeStringBuffer(PacketType type, String string) {

        ByteBuffer buffer = ByteBuffer.allocate(string.length() + 1);
    ...
    public static PeerData unpackPeerData(byte[] bytes, int length) {
        int[] sep = new int[2];
        int cnt, pos;
        for (pos = 0, cnt = 0; cnt < 2 && pos < 20 && pos < length; pos++) {
            if (bytes[pos] == ':') {
                sep[cnt++] = pos;
            }
        }
