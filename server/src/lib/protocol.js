...
const makePacketByType = {
  ...
  CALL_INIT: data => JSON.stringify({ s: data.status, m: data.message, i: data.sessionId, f: data.fromUser, a: data.address, p: data.port }),
...
const parsePacketByType = {
  ...
  CALL_INIT: payload => (d => ({ status: d.s, message: d.m, sessionId: d.i, fromUser: d.f, address: d.a, port: d.p }))(JSON.parse(payload.toString())),
